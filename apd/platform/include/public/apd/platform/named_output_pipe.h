...
class NamedOutputPipe : public NamedPipe {
 public:
  ...
  NamedOutputPipe(NamedOutputPipe&& other) : NamedPipe(std::move(other)) {
  }
  virtual ~NamedOutputPipe() {
  }
 public:
  template<typename SourceType> void write(const SourceType & data);
};

template<> void NamedOutputPipe::write<std::string>(const std::string & data);

template<typename SourceType> void NamedOutputPipe::write(const SourceType & data) {
