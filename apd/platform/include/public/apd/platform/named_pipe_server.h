...
class NamedPipeServer {
 ...
 public:
   ...
   */
  NamedPipeServer(const std::string & pathname, size_t message_size, size_t message_count, const std::string & client_pathname_template)
      : pipename_suffix_(client_pathname_template), pipe_((pathname), message_size * message_count, false) {
  }
   ...
   *         false - otherwise
   */
  template<typename SourceType> bool receive(pid_t & pid, SourceType & message) {
   ...
   *
   */
  template<typename SourceType> void send(const std::string & pathname, SourceType & message) {
      if (!client_pipe_) {
