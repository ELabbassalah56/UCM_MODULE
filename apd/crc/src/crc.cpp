...
uint32_t CRC::CalculateCRC32P4(BufferView bufferView, bool isFirstCall, uint32_t startValue) noexcept
{
    uint32_t crc;
    if (isFirstCall) {
...
uint32_t CRC::CalculateCRC32(BufferView bufferView, bool isFirstCall, uint32_t startValue) noexcept
{
    uint32_t crc;
    if (isFirstCall) {
        crc = startValueCrc32;
...
uint16_t CRC::CalculateCRC16(BufferView bufferView, bool isFirstCall, uint16_t startValue) noexcept
{
    uint16_t crc;
    if (isFirstCall) {
        crc = startValueCrc16;
...
uint64_t CRC::CalculateCRC64(BufferView bufferView, bool isFirstCall, uint64_t startValue) noexcept
{
    uint64_t crc;
    if (isFirstCall) {
        crc = startValueCrc64;
    ...
    for (const auto& byte : bufferView) {
        ...
        crc = ((crc >> 8U) & 0x00'FF'FF'FF'FF'FF'FF'FFU) ^ lookupTableCrc64[arrIdx];
    }
    crc ^= xorValueCrc64;
    return crc;
}
...
uint8_t CRC::CalculateCRC8H2F(BufferView bufferView, bool isFirstCall, uint8_t startValue) noexcept
{
    uint8_t crc;
    if (isFirstCall) {
...
uint64_t CRC::Reflect(uint64_t crc) noexcept
{
    ...
    uint64_t reflected{static_cast<uint64_t>(Reflect(subCrc0)) + (static_cast<uint64_t>(Reflect(subCrc1)) << 32U)};
    return reflected;
}
